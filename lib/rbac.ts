/**
 * RBAC roles and permissions for OpenMD.
 * Roles: admin (owner), doctor (provider), facility_manager (ops), billing, credentialing.
 * If you add permissions, also update public.tenant_role_has_permission (migration 20260310000300).
 */
...
/**
 * Permission matrix per role; `admin` gets everything.
 */
...
export function normalizeTenantRole(
  ...
  if (!role) return null;
  // Handle legacy role names stored in older rows.
