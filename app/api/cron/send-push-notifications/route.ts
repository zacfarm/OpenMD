...
/**
 * Cron endpoint for web push notifications (runs every 5 minutes).
 * Requires VAPID keys and SUPABASE_SERVICE_ROLE_KEY.
...
export async function GET(req: Request) {
  ...
  await Promise.allSettled(
    (rows as Row[]).map(async (row) => {
      // Fall back to the notifications page if no action URL was stored.
      const payload = JSON.stringify({
        title: row.title,
        body: row.body,
        url: row.action_url
          ? `${appUrl}${row.action_url}`
