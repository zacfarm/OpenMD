...
export default function SignupPage() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const inviteToken = String(
      data.get("inviteToken") || inviteTokenFromQuery,
    ).trim();
    ...
    if (!signUpData.user) {
      setBusy(false);
      setError(
        "Signup succeeded but no user was returned. Check auth settings.",
      );
      return;
    }
  ...
  return (
    <div className="auth-form-wrap">
      <header className="auth-heading">
        ...
        {isInviteSignup ? (
          <p>
            You are joining an existing OpenMD team. Create your account to
            activate your profile.
          </p>
        ) : (
          <p>
            Set up a practice or facility tenant with role-based access
            controls. Independent providers should register as a practice.
          </p>
        )}
      </header>
      ...
      <form onSubmit={onSubmit} className="auth-form">
        ...
        <p className="auth-help" style={{ marginTop: -6 }}>
