...
async function createInvite(formData: FormData) {
  ...
  if (
    !INVITABLE_TEAM_ROLES.includes(
      role as (typeof INVITABLE_TEAM_ROLES)[number],
    )
  ) {
  ...
  if (!membership || !hasPermission(membership.role, "manage_team")) {
    redirect(
      "/settings/team?error=You do not have permission to invite team members.",
    );
  }
  ...
  if (error) {
    const migrationHint = error.message.includes(
      "invalid input value for enum tenant_role",
    )
      ? " Run latest Supabase migrations, then retry."
      : "";

    redirect(
      `/settings/team?error=${encodeURIComponent(`${error.message}${migrationHint}`)}`,
    );
  }
...
async function expireInvite(formData: FormData) {
  ...
  if (!membership || !hasPermission(membership.role, "manage_team")) {
    redirect(
      "/settings/team?error=You do not have permission to expire invites.",
    );
  }

  const { error } = await supabase.rpc("expire_tenant_invite", {
...
export default async function TeamSettingsPage({
  ...
  const [membersRes, invitesRes] = await Promise.all([
    membership
      ? supabase
          .from("tenant_memberships")
          .select(
            "id,role,user_id,profiles!tenant_memberships_user_id_profile_fkey(email,full_name)",
          )
    ...
    membership
      ? supabase
          .from("tenant_invites")
          .select(
            "id,email,role,invite_token,status,created_at,opened_at,accepted_at,expires_at",
          )
          .eq("tenant_id", membership.tenant_id)
  ...
  const openedInvites = invites.filter(
    (invite) =>
      invite.status === "opened" && new Date(invite.expires_at) >= now,
  );
  ...
  const expiredInvites = invites.filter(
    (invite) =>
      invite.status !== "accepted" && new Date(invite.expires_at) < now,
  );
  ...
  return (
    <section style={{ display: "grid", gap: 14 }}>
      ...
      >
        ...
        <h1
          style={{ margin: "0 0 4px", fontSize: "clamp(1.4rem, 3vw, 1.9rem)" }}
        >
          Team Control Center
        </h1>
        <p style={{ margin: 0, color: "var(--muted)" }}>
          Manage roles, invitations, and workspace access for{" "}
          {membershipTenant?.name ?? "your organization"}.
        </p>
        ...
        >
          <div className="dashboard-mini-stat">
            ...
            <p style={{ margin: "8px 0 0", fontWeight: 700 }}>
              {getRoleLabel(membership?.role ?? "doctor")}
            </p>
          </div>
          <div className="dashboard-mini-stat">
      ...
      <div style={{ display: "grid", gap: 14, gridTemplateColumns: "2fr 1fr" }}>
        <article className="card" style={{ padding: 18 }}>
          ...
          <p
            style={{ color: "var(--muted)", margin: "0 0 14px", fontSize: 14 }}
          >
            Send role-based access invites for scheduling, credentialing,
            billing, and provider workflows.
          </p>

          {!process.env.NEXT_PUBLIC_APP_URL && (
            <p style={{ margin: "0 0 10px", color: "var(--warning)" }}>
              NEXT_PUBLIC_APP_URL is not set. Invite links will point to
              localhost.
            </p>
          )}
        ...
        <article className="card" style={{ padding: 18 }}>
          ...
          <p
            style={{ margin: "10px 0 0", color: "var(--muted)", fontSize: 12 }}
          >
            Use role labels to keep least-privilege access in place as your team
            grows.
          </p>
        </article>
      </div>
      ...
      <article className="card" style={{ padding: 18 }}>
        ...
        >
          ...
          <p style={{ margin: 0, color: "var(--muted)", fontSize: 13 }}>
            Role assignment and access visibility
          </p>
        </div>
        <div
          style={{
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    >
                      <Link
                        href={`/settings/team/${member.user_id}`}
                        style={{
                          color: "var(--accent)",
                          textDecoration: "none",
                        }}
      ...
      <article className="card" style={{ padding: 18 }}>
        ...
        <p style={{ color: "var(--muted)", marginTop: 0, marginBottom: 12 }}>
          Track invite state, resend emails for pending invites, and share
          direct signup links when needed.
        </p>
                ...
                >
                  ...
                  >
                    <div style={{ minWidth: 0 }}>
                      ...
                      >
                        {getRoleLabel(invite.role)} • Sent{" "}
                        {formatDateTime(invite.created_at)} • Expires{" "}
