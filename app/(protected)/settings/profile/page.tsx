...
async function updateProfile(formData: FormData) {
  ...
  const providerProfileId = String(
    formData.get("provider_profile_id") || "",
  ).trim();
  ...
  if (profileError) {
    redirect(
      `/settings/profile?error=${encodeURIComponent(profileError.message)}`,
    );
  }
  ...
  if (providerProfileId) {
    ...
    if (providerError) {
      redirect(
        `/settings/profile?error=${encodeURIComponent(providerError.message)}`,
      );
    }
  }
...
async function updateEmail(formData: FormData) {
  ...
  if (!email || !email.includes("@")) {
    redirect(
      "/settings/profile?emailError=Please enter a valid email address.",
    );
  }
  ...
  if (error) {
    redirect(
      `/settings/profile?emailError=${encodeURIComponent(error.message)}`,
    );
  }

  await logSecurityAction(user.id, "email_change_requested", {
    next_email: email,
  });

  revalidatePath("/settings/profile");
  redirect(
    "/settings/profile?emailSuccess=Email update requested. Check your inbox to confirm.",
  );
}
...
async function revokeOtherSessions() {
  ...
  if (error) {
    redirect(
      `/settings/profile?sessionError=${encodeURIComponent(error.message)}`,
    );
  }
  ...
  revalidatePath("/settings/profile");
  redirect(
    "/settings/profile?sessionSuccess=Other active sessions were signed out.",
  );
}
...
async function uploadAvatar(formData: FormData) {
  ...
  if (!(file instanceof File) || file.size === 0) {
    redirect(
      "/settings/profile?avatarError=Please choose an image file to upload.",
    );
  }

  if (file.size > 2 * 1024 * 1024) {
    redirect(
      "/settings/profile?avatarError=Avatar image must be 2MB or smaller.",
    );
  }
  ...
  if (!allowed.includes(ext)) {
    redirect(
      "/settings/profile?avatarError=Allowed image types: png, jpg, jpeg, webp.",
    );
  }
  ...
  if (uploadError) {
    redirect(
      `/settings/profile?avatarError=${encodeURIComponent(uploadError.message)}`,
    );
  }

  const { error: saveError } = await upsertUserSettings(user.id, {
    avatar_path: avatarPath,
  });
  if (saveError) {
    redirect(
      `/settings/profile?avatarError=${encodeURIComponent(saveError.message)}`,
    );
  }
...
async function updatePreferences(formData: FormData) {
  ...
  const digestFrequency = String(
    formData.get("digest_frequency") || "realtime",
  );

  const safePreferredContact: "email" | "phone" | "in_app" =
    preferredContact === "phone" || preferredContact === "in_app"
      ? preferredContact
      : "email";
  ...
  const { error } = await upsertUserSettings(user.id, {
    ...
    provider_license_state:
      String(formData.get("provider_license_state") || "").trim() || null,
    provider_license_number:
      String(formData.get("provider_license_number") || "").trim() || null,
    provider_board_certifications: parseCsv(
      String(formData.get("provider_board_certifications") || ""),
    ),

    billing_payer_focus: parseCsv(
      String(formData.get("billing_payer_focus") || ""),
    ),
    billing_certifications: parseCsv(
      String(formData.get("billing_certifications") || ""),
    ),

    credentialing_domains: parseCsv(
      String(formData.get("credentialing_domains") || ""),
    ),
    credentialing_regions: parseCsv(
      String(formData.get("credentialing_regions") || ""),
    ),

    linkedin_url: String(formData.get("linkedin_url") || "").trim() || null,
    publications_url:
      String(formData.get("publications_url") || "").trim() || null,
    cv_url: String(formData.get("cv_url") || "").trim() || null,
    credential_docs_url:
      String(formData.get("credential_docs_url") || "").trim() || null,
  });

  if (error) {
    redirect(
      `/settings/profile?prefsError=${encodeURIComponent(error.message)}`,
    );
  }
...
export default async function ProfileSettingsPage({
  ...
  const isCredentialing =
    roles.includes("credentialing") || roles.includes("scheduler");
  ...
  const onboardingItems: Array<{ label: string; done: boolean }> = [
    ...
    {
      label: "Phone and timezone set",
      done: Boolean(settings?.phone && settings?.timezone),
    },
    {
      label: "Preferred contact selected",
      done: Boolean(settings?.preferred_contact),
    },
  ];
  ...
  if (isDoctor) {
    onboardingItems.push(
      {
        label: "Provider display name and specialty",
        done: Boolean(
          providerProfile?.display_name && providerProfile?.specialty,
        ),
      },
      {
        label: "NPI and license details",
        done: Boolean(
          settings?.provider_npi && settings?.provider_license_number,
        ),
      },
      {
  ...
  if (isBilling) {
    onboardingItems.push(
      {
        label: "Billing payer focus entered",
        done: Boolean(
          settings?.billing_payer_focus &&
          settings.billing_payer_focus.length > 0,
        ),
      },
      {
        label: "Billing certification entered",
        done: Boolean(
          settings?.billing_certifications &&
          settings.billing_certifications.length > 0,
        ),
      },
    );
  }
  if (isCredentialing) {
    onboardingItems.push(
      {
        label: "Credentialing domains entered",
        done: Boolean(
          settings?.credentialing_domains &&
          settings.credentialing_domains.length > 0,
        ),
      },
      {
        label: "Credentialing regions entered",
        done: Boolean(
          settings?.credentialing_regions &&
          settings.credentialing_regions.length > 0,
        ),
      },
    );
  }
    ...
    <section style={{ display: "grid", gap: 14 }}>
      ...
      <article className="card" style={{ padding: 18 }}>
        ...
        <p style={{ marginTop: 0, color: "var(--muted)", fontSize: 14 }}>
          Your name appears in team settings, notifications, and activity
          summaries.
        </p>
          ...
          {providerProfile && (
            ...
            >
              ...
              <p style={{ margin: 0, color: "var(--muted)", fontSize: 13 }}>
                Keep your provider details accurate for scheduling, credentials,
        ...
        {(avatarError || avatarSuccess) && (
          <p
            style={{
              ...
              background: avatarError
                ? "rgba(180, 74, 46, 0.1)"
                : "rgba(11, 124, 69, 0.1)",
              border: avatarError
                ? "1px solid rgba(180, 74, 46, 0.28)"
                : "1px solid rgba(11, 124, 69, 0.28)",
              borderRadius: 10,
              padding: "8px 10px",
            }}
          >
          ...
          >
            ...
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <select
                ...
                defaultValue={settings?.preferred_contact ?? "email"}
              >
            ...
            <label style={{ display: "grid", gap: 6 }}>
              ...
              <select
                ...
                defaultValue={settings?.digest_frequency ?? "realtime"}
              >
          ...
          <div style={{ display: "grid", gap: 8 }}>
            ...
            >
              <input
                ...
                type="checkbox"
                defaultChecked={settings?.public_profile_visible ?? true}
              />{" "}
            ...
            >
              <input
                name="show_location"
                type="checkbox"
                defaultChecked={settings?.show_location ?? true}
              />{" "}
              Show city/state when available
            </label>
            <label
              style={{
                display: "inline-flex",
            ...
            >
              <input
                name="internal_only_contact"
                type="checkbox"
                defaultChecked={settings?.internal_only_contact ?? false}
              />{" "}
              Restrict contact details to internal users
            </label>
          </div>
            ...
            >
                ...
                >
                  <label style={{ display: "grid", gap: 6 }}>
                    ...
                    <input
                      className="field"
                      name="provider_npi"
                      defaultValue={settings?.provider_npi ?? ""}
                      maxLength={32}
                    />
                  </label>
                  <label style={{ display: "grid", gap: 6 }}>
                    ...
                    <input
                      ...
                      defaultValue={settings?.provider_license_state ?? ""}
                      maxLength={16}
                    />
                  </label>
                  <label style={{ display: "grid", gap: 6 }}>
                    ...
                    <input
                      ...
                      defaultValue={settings?.provider_license_number ?? ""}
                      maxLength={48}
                    />
                  </label>
                  <label
                  ...
                  >
                    ...
                    <input
                      className="field"
                      name="provider_board_certifications"
                      defaultValue={formatArray(
                        settings?.provider_board_certifications,
                      )}
                    />
                  </label>
                </div>
              ...
              {isBilling && (
                ...
                >
                  <label style={{ display: "grid", gap: 6 }}>
                    ...
                    <input
                      className="field"
                      name="billing_payer_focus"
                      defaultValue={formatArray(settings?.billing_payer_focus)}
                    />
                  </label>
                  <label style={{ display: "grid", gap: 6 }}>
                    <span style={{ fontSize: 13, color: "var(--muted)" }}>
              ...
              {isCredentialing && (
                ...
                >
                  <label style={{ display: "grid", gap: 6 }}>
                    <span style={{ fontSize: 13, color: "var(--muted)" }}>
                      Credentialing domains
                    </span>
                    <input
                      className="field"
                      name="credentialing_domains"
                      defaultValue={formatArray(
                        settings?.credentialing_domains,
                      )}
                    />
            ...
            >
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <input
                  ...
                  type="url"
                  defaultValue={settings?.publications_url ?? ""}
                />
              </label>
              <label style={{ display: "grid", gap: 6 }}>
                <span style={{ fontSize: 13, color: "var(--muted)" }}>
              ...
              <label style={{ display: "grid", gap: 6 }}>
                ...
                <input
                  className="field"
                  name="credential_docs_url"
                  type="url"
                  defaultValue={settings?.credential_docs_url ?? ""}
                />
              </label>
            </div>
          </div>
      ...
      <article className="card" style={{ padding: 18 }}>
        ...
        >
          <a className="btn btn-secondary" href="/api/profile/export">
            Download account export (CSV)
          </a>
        </div>
      </article>
