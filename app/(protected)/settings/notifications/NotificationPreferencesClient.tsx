...
const EVENT_TYPES = [
  ...
  { key: 'booking_status_changed',   label: 'Booking accepted / declined / canceled', group: 'Bookings' },
  { key: 'billing_claim_submitted',  label: 'Claim submitted',              group: 'Billing' },
  { key: 'billing_claim_status_changed', label: 'Claim accepted or rejected', group: 'Billing' },
...
const GROUPS = ['Bookings', 'Billing', 'Marketplace', 'Credentials', 'Team'] as const

export function NotificationPreferencesClient({ initialPrefs, userRoles = [] }: { initialPrefs: PrefsMap; userRoles?: TenantRole[] }) {
  ...
  const toggle = useCallback(
    async (eventType: string, field: 'in_app' | 'email') => {
      ...
      await supabase
        ...
        .upsert({ user_id: user.id, event_type: eventType, ...updated }, { onConflict: 'user_id,event_type' })
      setSaving(null)
    },
    [prefs]
  )
  ...
  return (
    <section className="card" style={{ padding: 18 }}>
      ...
      <p style={{ color: 'var(--muted)', marginTop: 0 }}>
        Choose which events you are notified about and by which channel. Changes save automatically.
      </p>
      ...
      {userRoles.length > 0 && (
        ...
        >
          ...
          <span style={{ color: 'var(--muted)', fontSize: 12 }}>
            You're seeing notification preferences relevant to your role(s). Contact your administrator if you need access to
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ borderBottom: '2px solid var(--line)' }}>
            <th style={{ textAlign: 'left', padding: '8px 4px', fontWeight: 600 }}>Event</th>
            <th style={{ textAlign: 'center', padding: '8px 4px', fontWeight: 600, width: 80 }}>In-app</th>
            <th style={{ textAlign: 'center', padding: '8px 4px', fontWeight: 600, width: 80 }}>Email</th>
          </tr>
        </thead>
        <tbody>
          {visibleGroups.map((group) => (
            <Fragment key={group}>
              ...
              {visibleEventTypes.filter((e) => e.group === group).map((ev) => {
                const pref = prefs[ev.key] ?? DEFAULT_PREF
                const isSaving = saving === ev.key
                return (
                  <tr key={ev.key} style={{ borderBottom: '1px solid var(--line)' }}>
                    <td style={{ padding: '10px 4px', fontSize: 14 }}>{ev.label}</td>
                    <td style={{ textAlign: 'center', padding: '10px 4px' }}>
                      <input
                        ...
                        style={{ cursor: isSaving ? 'default' : 'pointer', width: 16, height: 16 }}
                      />
                    </td>
                    <td style={{ textAlign: 'center', padding: '10px 4px' }}>
                      <input
      ...
      <p style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 0, marginTop: 16 }}>
        Email delivery requires a configured email service. Email notifications are sent in batches
        (up to once per hour).
      </p>
    </section>
  )
