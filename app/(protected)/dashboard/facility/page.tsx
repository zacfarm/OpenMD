...
export default async function FacilityDashboardPage() {
  ...
  const [{ data: providers }, { data: bookings }, { data: claims }] =
    await Promise.all([
      supabase
  ...
  for (let i = 7; i >= 0; i -= 1) {
    ...
    const utilizedThisWeek = new Set(
      (bookings ?? [])
        .filter((b) => {
          const bookingWeek = startOfUtcWeek(new Date(b.requested_start))
            .toISOString()
            .slice(0, 10);
    ...
    const utilizationPct =
      providerCount > 0
        ? Math.round((utilizedProviders / providerCount) * 100)
        : 0;
  ...
  for (let i = 5; i >= 0; i -= 1) {
    ...
    const monthRows = (claims ?? []).filter(
      (c) => c.submitted_at.slice(0, 7) === key,
    );
    const amount = monthRows.reduce(
      (sum, r) => sum + Number(r.billed_amount ?? 0),
      0,
    );
  ...
  const latestUtilization =
    weeklyPoints[weeklyPoints.length - 1]?.utilizationPct ?? 0;
  const latestUtilizedProviders =
    weeklyPoints[weeklyPoints.length - 1]?.utilizedProviders ?? 0;
  ...
  return (
    <section className="dashboard-shell">
      <article className="card dashboard-hero">
        <div>
          <p className="dashboard-eyebrow">Facility manager overview</p>
      ...
      <section className="dashboard-metric-grid">
        <article className="card metric-tile">
          ...
          <p className="metric-hint">
            {latestUtilizedProviders}/{providerCount} providers booked this week
          </p>
        </article>
        <article className="card metric-tile">
          ...
          <p className="metric-hint">
            Total billed amount across last 6 months
          </p>
        </article>
        <article className="card metric-tile">
      ...
      <section className="dashboard-two-col">
        <article className="card" style={{ padding: 18 }}>
          ...
          <p style={{ marginTop: 0, color: "var(--muted)" }}>
            Percent of providers with at least one accepted or confirmed booking
            each week.
          </p>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {weeklyPoints.map((p) => (
              <div
                ...
                style={{
                  display: "grid",
                  gridTemplateColumns: "72px 1fr 144px",
                  gap: 10,
                  alignItems: "center",
                }}
              >
                ...
                >
                  <div
                    style={{
                      ...
                      background:
                        "linear-gradient(90deg, #0c7a5a 0%, #18a878 100%)",
                    }}
                  />
                </div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {monthlyPoints.map((p) => (
              <div
                ...
                style={{
                  display: "grid",
                  gridTemplateColumns: "72px 1fr 188px",
                  gap: 10,
                  alignItems: "center",
                }}
              >
                ...
                >
                  <div
                    style={{
                      ...
                      background:
                        "linear-gradient(90deg, #1f4b99 0%, #4f7ad1 100%)",
                    }}
                  />
                </div>
                <span style={{ fontSize: 13 }}>
