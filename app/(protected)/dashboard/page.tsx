...
export default async function DashboardPage({
  ...
  const caseWindow =
    resolvedSearchParams?.caseWindow === "month" ? "month" : "today";
    ...
    await Promise.all([
      ...
      hasPermission(role, "view_billing") && activeMembership
        ? supabase
            .from("insurance_claims")
            .select(
              "id,patient_name,status,billed_amount,insurance_payers(payer_name)",
            )
            .eq("tenant_id", activeMembership.tenant_id)
  ...
  if (role === "facility_manager" && activeMembership) {
    ...
    if (providerIds.length > 0) {
      const [availabilityRes, credentialsRes, bookingsRes, claimsRes] =
        await Promise.all([
          supabase
      ...
      const availabilitySet = new Set(
        (availabilityRes.data ?? []).map((r) => r.provider_id),
      );
      ...
      utilizationRate =
        providerIds.length > 0
          ? Math.round((utilizedProvidersCount / providerIds.length) * 100)
          : 0;

      totalCost = claimCostRows.reduce(
        (sum, row) => sum + Number(row.billed_amount ?? 0),
        0,
      );
      ...
      avgClaimCost =
        claimCostRows.length > 0 ? totalCost / claimCostRows.length : 0;
      ...
      onboardingRows = (tenantProviders ?? [])
        ...
        .map((provider) => {
          const creds = providerCredentialRows.filter(
            (c) => c.provider_id === provider.id,
          );
          const providerBookings = providerBookingRows.filter(
          ...
          const items = [
            {
              ...
              done: Boolean(provider.display_name && provider.specialty),
            },
            { label: "At least 1 credential uploaded", done: creds.length > 0 },
            {
              label: "At least 1 credential approved",
              done: creds.some((c) => c.status === "approved"),
            },
            {
              label: "Availability added",
              done: availabilitySet.has(provider.id),
            },
            {
              label: "Booking activity started",
              done: providerBookings.length > 0,
            },
          ];
    ...
    <section className="dashboard-shell">
      ...
      <section className="dashboard-metric-grid">
        ...
        <article className="card metric-tile">
          <p className="metric-label">
            {role === "billing" ? "Claims submitted" : "Booking requests"}
          </p>
          <p className="metric-value">
            {role === "billing" ? (claimsCount ?? 0) : (bookingCount ?? 0)}
          </p>
        ...
        <section className="dashboard-two-col">
          ...
          <article className="card" style={{ padding: 18 }}>
            ...
            <p style={{ marginTop: 0, color: "var(--muted)" }}>
              High-level admin visibility across staffing, billing, and team
              operations.
            </p>
            <div
              style={{
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        ...
                        {formatMoney(claim.billed_amount)} • {claim.status}
                      </p>
                    </article>
                  );
                })}
        ...
        <article className="card" style={{ padding: 18 }}>
          <div className="section-head">
            ...
            <div className="dashboard-case-window">
              <a
                className={`btn ${caseWindow === "today" ? "btn-primary" : "btn-secondary"}`}
                href="/dashboard?caseWindow=today"
              >
                Today
              </a>
              <a
                className={`btn ${caseWindow === "month" ? "btn-primary" : "btn-secondary"}`}
                href="/dashboard?caseWindow=month"
              >
                This Month
              </a>
            </div>
          ...
          <div className="dashboard-case-table">
            ...
            {pendingCases.length === 0 ? (
              <div className="dashboard-case-empty">
                No open cases in the{" "}
                {caseWindow === "month" ? "current month" : "current day"}.
              </div>
            ) : (
              pendingCases.map((event) => (
                ...
                >
                  <div className="dashboard-case-primary">
                    ...
                    <span>
                      {event.caseIdentifier
                        ? `Case ${event.caseIdentifier}`
                        : event.procedureType?.name ||
                          event.caseType ||
                          "Open case"}
                    </span>
                  ...
                  <div>
                    <span
                      className={`dashboard-case-status dashboard-case-status-${event.status.replace(/_/g, "-")}`}
                    >
                  ...
                  <div className="dashboard-case-meta">
                    ...
                    {event.provider?.specialty && (
                      <span>{event.provider.specialty}</span>
                    )}
                  </div>
                  ...
                  <div className="dashboard-case-actions">
                    ...
                    <a
                      ...
                      href={getCalendarBillingHref(event)}
                    >
                      Billing
                    </a>
                  </div>
        ...
        >
          <article className="card" style={{ padding: 18 }}>
            ...
            <p style={{ color: "var(--muted)" }}>
              Claim posted shifts with one click and fill your schedule faster.
            </p>
            <div style={{ display: "grid", gap: 10 }}>
              {(openFacilityShifts ?? []).length === 0 ? (
                <p style={{ margin: 0, color: "var(--muted)" }}>
                  No open facility shifts are available right now.
                </p>
              ) : (
                (openFacilityShifts ?? []).map((post) => (
                  <article key={post.id} className="dashboard-shift-row">
                    <div>
                      ...
                      >
                        {new Date(post.starts_at).toLocaleDateString()}{" "}
                        {post.ends_at
                          ? `to ${new Date(post.ends_at).toLocaleDateString()}`
                          : ""}
                      </p>
                    </div>
          ...
          <article className="card" style={{ padding: 18 }}>
            ...
            <p style={{ marginTop: 0, color: "var(--muted)" }}>
              Everything you need to stay booked and compliant.
            </p>
      ...
      {role === "billing" && (
        <article className="card" style={{ padding: 18 }}>
          ...
          <p style={{ color: "var(--muted)" }}>
            Select payers from the configured database and submit claims from
            the billing portal.
          </p>
              ...
              claimRows.map((claim) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p style={{ margin: "4px 0", color: "var(--muted)" }}>
                      {payer?.payer_name ?? "Unknown payer"} | $
                      {claim.billed_amount.toFixed(2)} | {claim.status}
                    </p>
                  </div>
                );
              })
      ...
      {role === "credentialing" && (
        <article className="card" style={{ padding: 18 }}>
          ...
          <p style={{ color: "var(--muted)" }}>
            Manage bookings, provider readiness, and open staffing requests from
            one place.
          </p>
          <div
            style={{
          ...
          >
            <article className="card" style={{ padding: 14 }}>
              ...
              <p style={{ margin: "8px 0 0", fontSize: 26 }}>
                {bookingCount ?? 0}
              </p>
            </article>
            <article className="card" style={{ padding: 14 }}>
        ...
        <>
          <article className="card" style={{ padding: 18 }}>
            <h2 style={{ marginTop: 0 }}>
              Staff utilization and cost dashboard
            </h2>
            <p style={{ color: "var(--muted)" }}>
              Snapshot of provider engagement and billing spend for your
              facility.
            </p>
            ...
            >
              <article className="card" style={{ padding: 14 }}>
                ...
                >
                  {utilizedProvidersCount} of {providerCount ?? 0} providers
                  with accepted/confirmed bookings
                </p>
              </article>
          ...
          <article className="card" style={{ padding: 18 }}>
            ...
            <p style={{ color: "var(--muted)" }}>
              Tracks providers created in the last 60 days and their onboarding
              progress.
            </p>

            {onboardingRows.length === 0 ? (
              <p style={{ margin: 0, color: "var(--muted)" }}>
                No newly added providers in the last 60 days.
              </p>
            ) : (
              <div style={{ display: "grid", gap: 10 }}>
                      ...
                      {row.items.map((item) => (
                        <p
                          ...
                          style={{
                            margin: 0,
                            fontSize: 13,
                            color: item.done ? "var(--accent)" : "var(--muted)",
                          }}
                        >
